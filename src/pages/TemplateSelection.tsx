    ...
    if (key === 'photo' && value.includes('.jpg')) {
      return (
        <div className="flex items-center gap-2">
          ...
          <Badge variant="secondary" className="text-xs">{t('common.image')}</Badge>
        </div>
      );
    }
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-bold">{t('pages.mapping.title')}</h1>
          <p className="text-muted-foreground">{t('pages.mapping.description')}</p>
        </div>
      </div>
      ...
      <Card className="shadow-soft">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            {t('pages.mapping.recordSelection')}
          </CardTitle>
          <CardDescription>
            {t('pages.mapping.recordSelectionDesc')}
          </CardDescription>
        </CardHeader>
        <CardContent>
                      ...
                      <div className="mt-4 p-3 bg-muted/20 rounded-lg">
                        <p className="text-xs text-muted-foreground">
                          {selectedRecord === record.id
                            ? t('pages.mapping.recordWillBeMapped')
                            : t('pages.mapping.selectRecordToMap')}
                        </p>
                      </div>
                    </div>
                  </AccordionContent>
      ...
      <div className="flex justify-between">
        <Button variant="outline">
          <Link to='/preview'>
            {t('pages.mapping.backToDataPreview')}
          </Link>
        </Button>
        <Button className="bg-gradient-primary shadow-elegant">
          <Link to='/edit'>
            {t('pages.mapping.continueToEditData')}
          </Link>
        </Button>
      </div>
    </div>
